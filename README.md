...
- **구글 캘린더 · 구글 할 일**: 이용자의 컴퓨터에만 저장되며, 개발자에게 전송되거나
  개발자가 보관하지 않습니다
- **로그인 표(OAuth 토큰)**: 윈도우 DPAPI 로 암호화해 저장하며, 다른 계정이나 다른
  컴퓨터에서는 열 수 없습니다
- **네트워크 통신**: 구글 캘린더 · 구글 할 일 API, 업데이트 확인(GitHub),
  그리고 **AI 일정 입력을 쓸 때에 한해** 구글 Gemini 와 API 키를 받는 중계 서버
- **데이터 수집**: 이용 패턴을 수집하거나 분석 도구를 넣지 않습니다

자세한 내용은 [개인정보처리방침](https://d-deskcal.duckdns.org/privacy.html)을 참조하세요.
