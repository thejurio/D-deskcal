...
# views/base_view.py 파일의 BaseViewWidget 클래스
    ...
    def show_popover(self):
        ...
        # 팝오버 신호 연결
        logger.debug(f"Connecting popover signals for: {self.hovered_event_data.get('summary', '')}")
        ...
        logger.debug("Popover signals connected")
        ...
        # 디버그 정보 추가
        logger.debug(f"Setting popover position - x={x}, y={y}, size={popover_size}")
        # Removed excessive cursor position logging
        # Removed excessive screen size logging
        
        self.current_popover.show()
        logger.debug("Popover show() completed")
        logger.debug(f"Popover visible={self.current_popover.isVisible()}, pos={self.current_popover.pos()}")
